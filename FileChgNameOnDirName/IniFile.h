...
class CIniFile  
{
	CArray<SECTION_STRUCT,SECTION_STRUCT> m_Section;		
public:
	...
	int GetInt(CString Section, CString Item, int Value);		// 最后Value为,当读取失败时的默认值
	...
	CStdioFile IniFile;
	BOOL  bFileExist;
	CStringArray m_csFileContainer;
	CStringArray m_csAllSections;
};
